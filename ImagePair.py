...
class ImagePair(object):
    ...
    def vectorIntersction(self, cameraPoints1, cameraPoints2):
        ...
        for i in range(len(cameraPoints1)):
            # compute rays
            # v1 = np.dot(R1, np.array([[cameraPoints1[i, 0] - image1.camera.principalPoint[0],
            #                           cameraPoints1[i, 1] - image1.camera.principalPoint[1],
            #                           -image1.camera.focalLength]]).T)
            # v2 = np.dot(R2, np.array([[cameraPoints2[i, 0] - image2.camera.principalPoint[0],
            #                           cameraPoints2[i, 1] - image2.camera.principalPoint[1],
            #                           -image2.camera.focalLength]]).T)
            v1 = np.dot(R1, np.array([[cameraPoints1[i, 0],
                                       cameraPoints1[i, 1],
                                       -image1.camera.focalLength]]).T)
